<template>
  <div class="font-[sans-serif] max-w-4xl flex items-center mx-auto md:h-screen p-4" data-aos="fade-up"
     data-aos-duration="3000">
    <div class="grid bg-gray-100 md:grid-cols-3 items-center rounded-xl overflow-hidden">
      <div class="max-md:order-1 shadow-gray-100 flex flex-col justify-center space-y-16 max-md:mt-16 min-h-full animate-gradient h-full lg:px-8 px-4 py-4">
        <div>
      ...
      <form @submit.prevent="handleSubmit" class="md:col-span-2 w-full py-6 px-6 sm:px-16">
        ...
        <div class="!mt-12">
          <button type="submit" class="w-full py-3 px-4 animate-gradient tracking-wider text-sm rounded-md text-white bg-gray-700 hover:bg-gray-800 focus:outline-none">
            Login
          </button>
        </div>
        <p class="text-gray-800 text-sm mt-6 text-center">Don't have an account? <nuxt-link to="/signup" class="text-blue-600 font-semibold hover:underline ml-1">Sign up here</nuxt-link></p>
      </form>
    </div>
  </div>
</template>
...
.animate-gradient {
  background: linear-gradient(270deg, #181717, #3f3d3d, #948e8e, #d4cfc5);
